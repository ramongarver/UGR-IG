...
{
  glPushMatrix();
    if (activo[0] == 1) tronco.draw(modo, r_seleccion, g_seleccion, b_seleccion, r2, g2, b2, grosor);
    else                tronco.draw(modo, 0.4588, 0.9255, 0.6196, r2, g2, b2, grosor);
    
    glPushMatrix();
      glRotatef(giro_cuello,0,1,0);
      glPushMatrix();
        glTranslatef(0.0, tronco.altura/2 + cuello.altura/2, 0.0);
        if (activo[1] == 1) cuello.draw(modo, r_seleccion, g_seleccion, b_seleccion, r2, g2, b2, grosor);
        else                cuello.draw(modo, 0.8549, 0.8549, 0.8549, r2, g2, b2, grosor);
        glTranslatef(0.0, cuello.altura/2 + cabeza.altura/2, 0.0);
        if (activo[2] == 1) cabeza.draw(modo, r_seleccion, g_seleccion, b_seleccion, r2, g2, b2, grosor);
        else                cabeza.draw(modo, 0.0, 0.8392, 0.8392, r2, g2, b2, grosor);
      glPopMatrix();
    glPopMatrix();

    glPushMatrix();
      glTranslatef(0.125, -tronco.altura/2 -0.15*pierna_izd.altura/2, 0.0);
      if (activo[3] == 1) pierna_izd.draw(modo, r_seleccion, g_seleccion, b_seleccion, r2, g2, b2, grosor);
      else                pierna_izd.draw(modo, 0.9961, 0.7843, 0.8471, r2, g2, b2, grosor);
    glPopMatrix();

    glPushMatrix();
      glTranslatef(-0.125, -tronco.altura/2 -0.15*pierna_dch.altura/2, 0.0);
      if (activo[4] == 1) pierna_dch.draw(modo, r_seleccion, g_seleccion, b_seleccion, r2, g2, b2, grosor);
      else                pierna_dch.draw(modo, 0.9961, 0.7843, 0.8471, r2, g2, b2, grosor);
    glPopMatrix();

    glPushMatrix();
      glTranslatef(tronco.anchura/2 + 1.2*brazo_izd.anchura/2, 0.1875, 0.0);
      if (activo[5] == 1) brazo_izd.draw(modo, r_seleccion, g_seleccion, b_seleccion, r2, g2, b2, grosor);
      else                brazo_izd.draw(modo, 1.0, 0.8706, 0.502, r2, g2, b2, grosor);
    glPopMatrix();

    glPushMatrix();
      glTranslatef(-tronco.anchura/2 - 1.2*brazo_dch.anchura/2, 0.1875, 0.0);
      if (activo[6] == 1) brazo_dch.draw(modo, r_seleccion, g_seleccion, b_seleccion, r2, g2, b2, grosor);
      else                brazo_dch.draw(modo, 1.0, 0.8706, 0.502, r2, g2, b2, grosor);
    glPopMatrix();
    
  glPopMatrix();
}

void _robot::select(float c)
{
  glPushMatrix();
    tronco.draw(SELECT, c, c, c, c, c, c, 1.0); c+=20;
    
    glPushMatrix();
      glRotatef(giro_cuello,0,1,0);
      glPushMatrix();
        glTranslatef(0.0, tronco.altura/2 + cuello.altura/2, 0.0);
        cuello.draw(SELECT, c, c, c, c, c, c, 1.0); c+=20;
        glTranslatef(0.0, cuello.altura/2 + cabeza.altura/2, 0.0);
        cabeza.draw(SELECT, c, c, c, c, c, c, 1.0); c+=20;
      glPopMatrix();
    glPopMatrix();

    glPushMatrix();
      glTranslatef(0.125, -tronco.altura/2 -0.15*pierna_izd.altura/2, 0.0);
      pierna_izd.draw(SELECT, c, c, c, c, c, c, 1.0); c+=20;
    glPopMatrix();

    glPushMatrix();
      glTranslatef(-0.125, -tronco.altura/2 -0.15*pierna_dch.altura/2, 0.0);
      pierna_dch.draw(SELECT, c, c, c, c, c, c, 1.0); c+=20;
    glPopMatrix();

    glPushMatrix();
      glTranslatef(tronco.anchura/2 + 1.2*brazo_izd.anchura/2, 0.1875, 0.0);
      brazo_izd.draw(SELECT, c, c, c, c, c, c, 1.0); c+=20;
    glPopMatrix();

    glPushMatrix();
      glTranslatef(-tronco.anchura/2 - 1.2*brazo_dch.anchura/2, 0.1875, 0.0);
      brazo_dch.draw(SELECT, c, c, c, c, c, c, 1.0);
    glPopMatrix();
